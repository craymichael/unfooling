...
def one_hot_encode(y):
    """ One hot encode y for binary features.  We use this to get from 1 dim ys
    to predict proba's.
    This is taken from this s.o. post:
...
def rank_features(explanation):
    ...
    results = [tup[0] if tup[1] != 0 else ("Nothing shown", 0)
               for tup in ordered_tuples]
    return results


def get_rank_map(ranks, to_consider, k=None):
    """ Give a list of feature names in their ranked positions, return a map
    from position ranks to pct occurrences.
    ...
    for i, rank in enumerate(ranks):
        for unique_rank in np.unique(rank):
            unique[i + 1].append(
                (unique_rank,
                 np.sum(np.array(rank) == unique_rank) / to_consider))
        unique[i + 1] = sorted(unique[i + 1], key=lambda x: x[1], reverse=True)
        if k is not None:
            unique[i + 1] = unique[i + 1][:k]

    return unique


def experiment_summary(explanations, features, k=None):
    """ Provide a high level display of the experiment results for the top three
    features. This should be read as the rank (e.g. 1 means most important) and
    the pct occurrences of the features of interest.
    ...
    return get_rank_map(top_features, len(explanations), k=k)
