...
DATA_BASE = Path(fooling.__file__).parent.parent / 'data'


def get_and_preprocess_compas_data(params):
    """Handle processing of COMPAS according to:
    ...
    Returns
    ----------
    Pandas data frame X of processed data, np.ndarray y, and list of column
    names
    """
    ...
    compas_df = pd.read_csv(DATA_BASE / "compas-scores-two-years.csv",
                            index_col=0)
    ...
    compas_df['length_of_stay'] = (
            pd.to_datetime(compas_df['c_jail_out']) - pd.to_datetime(
        compas_df['c_jail_in'])).dt.days
    X = compas_df[['age', 'two_year_recid', 'c_charge_degree', 'race', 'sex',
                   'priors_count', 'length_of_stay']]

    # if person has high score give them the _negative_ model outcome
    y = np.array(
        [NEGATIVE_OUTCOME if score == 'High' else POSITIVE_OUTCOME for score in
...
def get_and_preprocess_cc(params):
    """"Handle processing of Communities and Crime.  We exclude rows with
    missing values and predict if the violent crime is in the 50th percentile.
    ...
    X = pd.read_csv(DATA_BASE / "communities_and_crime_new_version.csv",
                    index_col=0)

    # everything over 50th percentile gets negative outcome (lots of crime is
    # bad)
    ...
    X = X.drop(
        cols_with_missing_values + ['communityname string', 'fold numeric',
                                    'county numeric', 'community numeric',
                                    'state numeric'] + [y_col], axis=1)

    # setup ys
    cols = [*X.keys()]
    y = np.array(
        [NEGATIVE_OUTCOME if val > y_cutoff else POSITIVE_OUTCOME for val in y])

    return X, y, cols


def get_and_preprocess_german(params):
    """Handle processing of German.  We use a preprocessed version of German
    from Ustun et. al.
    ...
    X = pd.read_csv(DATA_BASE / "german_processed.csv")
    y = X["GoodCustomer"]
    ...
    y = np.array(
        [POSITIVE_OUTCOME if p == 1 else NEGATIVE_OUTCOME for p in y.values])
